...
def PreProcess(sentence, sentment):
    file_path = "DataSets\TrainingDataSet.csv"
    df = pd.read_csv(file_path, encoding="utf-8")
    df = pd.concat([df, pd.DataFrame([[sentment, sentence]],columns=['class', "text"])])
    ...
    df.to_csv("DataSets\CleanedTrainingDataSetLOCAL.csv", encoding="utf-8",index=False)
    return df
